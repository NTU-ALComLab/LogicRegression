...
*/
				...
				if (strcmp(Mio_GateReadName(pGate), "zero") == 0) {
					/* fprintf( pFile, "  buf  g%0*d(%s, 1'b0);\n", nDigits, Counter++, Io_WriteVerilogGetName(Abc_ObjName( Abc_ObjFanout0(pObj) )) ); */
					fprintf( pFile, "  buf (%s, 1'b0);\n", Io_WriteVerilogGetName(Abc_ObjName( Abc_ObjFanout0(pObj) )) );
					continue;
				}

				else if (strcmp(Mio_GateReadName(pGate), "one") == 0) {
					/* fprintf( pFile, "  buf  g%0*d(%s, 1'b1);\n", nDigits, Counter++, Io_WriteVerilogGetName(Abc_ObjName( Abc_ObjFanout0(pObj) )) ); */
					fprintf( pFile, "  buf (%s, 1'b1);\n", Io_WriteVerilogGetName(Abc_ObjName( Abc_ObjFanout0(pObj) )) );
					continue;
				}
